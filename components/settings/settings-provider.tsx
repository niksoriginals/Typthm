...
import { syncTypthmFavicon } from "@/lib/favicon-client";
...
interface SettingsContextType {
  ...
  setSparksEnabled: (v: boolean) => void;
...
function applyAccentToDom(accent: KeyboardThemeName) {
  ...
  queueMicrotask(() => syncTypthmFavicon());
}
...
export function SettingsProvider({ children }: { children: ReactNode }) {
  const [accent, setAccentState] = useState<KeyboardThemeName>("aurora");
  ...
  const [sparksEnabled, setSparksEnabledState] = useState(true);
  ...
  useEffect(() => {
    ...
    const savedSparksEnabled = localStorage.getItem("tc-sparks-enabled");
    ...
    if (savedSparksEnabled !== null) {
      setSparksEnabledState(savedSparksEnabled === "true");
    }
  ...
  const setSparksEnabled = (v: boolean) => {
    setSparksEnabledState(v);
    localStorage.setItem("tc-sparks-enabled", String(v));
  };
