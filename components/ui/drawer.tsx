...
const drawerPopupClassName = ({
  ...
  return cn(
    ...
    "touch-auto overflow-y-auto overscroll-contain bg-background/55 text-foreground outline-1 outline-foreground/5 [--bleed:3rem] data-swiping:select-none backdrop-blur-[32px] dark:bg-[oklch(0.141_0.005_285.823)/0.55]",
...
function DrawerBackdrop({
  ...
  return (
    <DrawerPrimitive.Backdrop
      className={(state) =>
        cn(
          "fixed inset-0 z-100 min-h-dvh bg-black opacity-[calc(var(--backdrop-opacity)*(1-var(--drawer-swipe-progress)))] transition-all duration-450 ease-[cubic-bezier(0.32,0.72,0,1)] [--backdrop-opacity:0.45] data-ending-style:opacity-0 data-starting-style:opacity-0 backdrop-blur-[6px] data-ending-style:backdrop-blur-none data-starting-style:backdrop-blur-none",
          resolveClassName(className, state)
        )
      }
