...
import { triggerSparks } from "@/lib/sparks";
...
export function TypingTest(props: TypingTestProps) {
  const { liveStats, faahMode, ghostMode, sparksEnabled } = useSettings();
  ...
  const handleKeyDownWithSparks = useCallback((e: React.KeyboardEvent<HTMLInputElement>) => {
    if (sparksEnabled && (e.key.length === 1 || e.key === "Backspace")) {
      triggerSparks();
    }
    handleKeyDown(e);
  }, [handleKeyDown, sparksEnabled]);
      ...
      <div className="relative w-full">
        ...
        >
          <input
            ...
            onKeyDown={handleKeyDownWithSparks}
            ref={inputRef}
            spellCheck={false}
            value={typed}
          />
